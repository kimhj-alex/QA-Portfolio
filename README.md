...
│   └── gemini-audit.html             # Gemini Usability & Reliability Audit (5 findings)
├── bug-reports/
...
Four structured platform audits form the centerpiece of the manual testing side of this portfolio.
...
**Audit 04 — Gemini Usability & Reliability Audit** · 5 findings  
Findings from real asset-generation work for a personal visual novel project, across image and video generation and app reliability — a style reference progressively substituted for the actual requested characters, a guided workflow that invited an unsupported action before rejecting it twice, video clips labeled "silent" that instead contained fabricated, translatable speech and sound design, and a chat list that disagreed with itself across two views of the same account.
...
|---|---|---|---|
| BR-001 | Instagram | Ad pricing information incomplete on mobile compared to desktop | High |
...
| BR-004 | Squarespace / Gmail | Form submissions thread into a single Gmail conversation with no documented fix | Low |
| BR-005 | Etsy | No category exists for wireless earbud cases — sellers forced to misclassify | Low |
| BR-006 | Amazon KDP | Category conflict warning provides no explanation, rule, or actionable guidance | Medium |
| BR-007 | YouTube Studio | Music content detection inconsistent with no manual override or creator control | High |
...
Every finding in this portfolio was discovered during actual use — not on practice bug-tracking sites, demo environments, or adversarial "red-team" prompts designed to break something. The bugs and Gen AI findings here are the ones that interrupted real work: publishing on Etsy, running ads for a small business, studying for a certification exam, building and deploying a web app, writing and publishing novellas, planning a real service, generating assets for a visual novel.
...
The Gemini audit follows the same standard as the case studies above, applied to a structured, multi-finding format: no claim — including the tool's own explanation of its behavior — was accepted without checking it against what was actually asked, what the tool said elsewhere in the same conversation, or an independent source (translation, source screenshots, a second device).

Testing covered web, mobile (iOS), iPad, macOS, and cross-browser environments including Chrome, Brave, and Safari. The Shoebox Planner audit used checklist testing, exploratory testing, and cross-device regression testing across desktop and iPad.
